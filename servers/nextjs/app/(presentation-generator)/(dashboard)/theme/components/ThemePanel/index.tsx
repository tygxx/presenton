...
const ThemePanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadCustomThemes = async () => {
      try {
        ...
        notify.error(
          '无法加载主题',
          error?.message || '已保存的主题加载失败，但内置主题仍可使用。'
        )
      }
    }
  ...
  const handleBrandLogoUpload = async (file: File) => {
    try {
      ...
      notify.error(
        '无法上传 Logo',
        error?.message || '上传 Logo 时出现问题，请重试。'
      )
    } finally {
  ...
  const saveAsCustom = async () => {
    ...
    if (selectedTheme.user && selectedTheme.user !== 'system' && !selectedTheme.id.startsWith('custom-')) {
      ; (async () => {
        try {
          ...
          notify.success('主题已更新', '主题修改已保存。')
        } catch (error: any) {
          console.error('Failed to update theme', error)
          notify.error(
            '无法更新主题',
            error?.message || '保存主题修改时出错。'
          )
        }
      })()
      return
    ...
    try {
      ...
      notify.success('主题已保存', '新主题已创建，可以使用了。')
    } catch (error: any) {
  ...
  const handleDelete = async (themeId: string) => {
    try {
      ...
      notify.success("主题已删除", "该主题已从主题库中移除。")
    } catch (error: any) {
    ...
    >
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          {step === 2 && <p className="text-xs text-[#4C4C4C] font-medium rounded-lg text-end pb-1.5">品牌色</p>}
          <div className="space-y-4"
            style={{
        ...
        {step === 2 && <div>
          <p className="text-xs text-[#4C4C4C] font-medium text-end pb-1.5">文字色</p>
          <div className="space-y-4"
            style={{
              padding: step === 2 ? '10px' : '0px',
              backgroundColor: step === 2 ? '#F9FAFB' : 'transparent'
        ...
        {step === 2 && <div>
          <p className="text-xs text-[#4C4C4C] font-medium text-end pb-1.5">图表色</p>
          <div className="space-y-4"
            style={{
              padding: step === 2 ? '10px' : '0px',
              backgroundColor: step === 2 ? '#F9FAFB' : 'transparent'
            ...
            <>
              ...
              <div className="mt-2">
                <label htmlFor="logo-upload" className="cursor-pointer">
                  ...
                  <span className="text-gray-500">，或拖放到此</span>
                </label>
                <input
                  id="logo-upload"
                  type="file"
                ...
                <div className='pt-7  w-full  h-[calc(90vh-120px)] flex flex-col justify-between  '>
                  ...
                  <div className='border-t border-[#EDEEEF] py-6 '>
                    <div className='flex justify-end px-5 gap-2'>
                      ...
                      >
                        {currentStep === 1 ? '生成配色方案' : currentStep === 2 ? '下一步：字体' : currentStep === 3 ? '下一步：设计' : '保存为自定义主题'}
                        <ChevronRight className='w-4 h-4' />
                      </button>
                    </div>
                  </div>

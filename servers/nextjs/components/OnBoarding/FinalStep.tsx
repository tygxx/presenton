    ...
    return (
        <div className='fixed top-0 left-0 w-full h-full flex flex-col items-center justify-center'>
            <div className='flex flex-col items-center justify-center'>
                ...
                <p className='text-[#000000CC] text-xl font-normal font-syne'>一切就绪，开始创建你的第一个演示文稿吧。</p>

                {trackingEnabled !== null && (
                    <div className='flex items-center gap-3 mt-8 px-5 py-3.5 rounded-[10px] border border-[#EDEEEF] bg-white'>
                        <div>
                            <p className='text-sm font-medium text-[#191919] font-syne'>使用数据上报</p>
                            <p className='text-[11px] text-[#9CA3AF] font-syne leading-tight mt-0.5'>通过匿名使用数据帮助改进 Presenton。</p>
                        </div>
                        <Switch
                ...
                <button onClick={handleGoToUpload} className='bg-[#7C51F8] px-[23px] mt-8 py-[15px]  rounded-[70px] text-white text-lg font-syne font-semibold'>创建我的第一个演示文稿 🚀</button>
                <button onClick={fireRealisticConfetti} className='mt-3 flex items-center gap-1.5 text-sm text-[#7A5AF8] font-syne font-medium hover:underline'>
                    <PartyPopper className='w-4 h-4' /> 再来一次庆祝！
                </button>
            </div>
            <button onClick={handleGoToDashboard} className='absolute uppercase bottom-20 text-[#7A5AF8] flex items-center gap-2 right-10  text-xs font-normal font-syne'>进入仪表盘 <ArrowRight className='w-4 h-4 text-[#7A5AF8]' /></button>
        </div>
    )
}
